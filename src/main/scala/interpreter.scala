...
package love_letter {
  ...
  package game {
    ...
    class Interpreter(choiceFns: Map[Player, Choice]) { 
      ...
      def playContinuation(state: State) = state match {
        case State(ActualCard(c), players@(p :: rest), deck, k) =>
          import cards._
    ...
    def next(s: State): Either[State, Over] = s match {
      ...
      case State(Bottom, players, _, DrawK(p) :: k) =>
        ...
        val cts = 
          if(pc.has(cards.Countess) && (pc.has(cards.King) || pc.has(cards.Prince)))
      ...
      case s@State(ActualCard(c), players@(p :: rest), deck, k) =>
        playContinuation(s)
    }
  }
}
